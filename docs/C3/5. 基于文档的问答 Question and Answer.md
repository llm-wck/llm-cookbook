...
基于文档问答的这个过程，我们会涉及 LangChain 中的其他组件，比如：`嵌入模型（Embedding Models）`和`向量储存（Vector Stores）`，本章让我们一起来学习这部分的内容。
...
#因为文档比较短，所以这里不需要进行任何分块,可以直接进行向量表征
...
通过 LangChain 创建一个检索问答链，对检索到的文档进行问题回答。检索问答链的输入包含以下
- `llm`: 语言模型，进行文本生成
